...
public abstract class MicroService implements Runnable {
    ...
    private MessageBusImpl _messageBus = MessageBusImpl.getInstance();
    private HashMap <Request<?> , Callback> _reqMap = new HashMap <Request<?> , Callback>();
    private HashMap <Class<? extends Message>, Callback> _typeMap = new HashMap <Class<? extends Message>, Callback>();    
    ...
    protected final <R extends Request> void subscribeRequest(Class<R> type, Callback<R> callback) {
    	_typeMap.put(type,callback);    
    	_messageBus.subscribeRequest(type, this);  
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
    	_typeMap.put(type,callback);    	
    	_messageBus.subscribeBroadcast(type, this); 
    }
    ...
    protected final <T> boolean sendRequest(Request<T> r, Callback<T> onComplete) {
    	boolean ans =  _messageBus.sendRequest(r, this);
    	if (ans) {
    		_reqMap.put(r, onComplete);
    	}    	
    	return ans;
    }
    ...
    public final void run() {
        ...
        while (!_terminated) {
        	try {
				Message nextMes =  _messageBus.awaitMessage(this);
				if (nextMes.getClass() == RequestCompleted.class) {
					RequestCompleted completedRequest= ((RequestCompleted)nextMes); 
					_reqMap.get(completedRequest.getCompletedRequest()).call(completedRequest.getResult()); 
				}
				else {
						_typeMap.get(nextMes.getClass()).call(nextMes);
				}
        	}
